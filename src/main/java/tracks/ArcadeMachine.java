...
public class ArcadeMachine {
	...
	/**
	 * Runs a single instance of a game, with variable agent parameters.
	 * Only single-player currently supported.
	 */
	public static double[] runMFMCTSGame(String game_file, String level_file, boolean visuals, String agentName,
										 Configuration configuration, int randomSeed, int playerID, String actionFile) {
		VGDLFactory.GetInstance().init(); // This always first thing to do.
		VGDLRegistry.GetInstance().init();
		...
		//Instantiate players. Single-player only
		int no_players = 1;
		AbstractPlayer[] players = new AbstractPlayer[no_players];

		for (int i = 0; i < no_players; i++) {

			players[i] = ArcadeMachine.createPlayer(agentName, actionFile, toPlay.getObservation(), randomSeed, false);
			((Agent) players[i]).Configure(configuration);
		}
